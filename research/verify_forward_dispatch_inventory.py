...
ROWS: list[Row] = [
    ...
    Row("gdn", "fused in-proj quantizedMM [qkv|z|b|a]", N_GDN, qmv,
        f"{Q35}:675", "quantizedMM("),
    Row("gdn", "z reshape copy off the packed in-proj slice", N_GDN, K(1),
        f"{Q35}:981", "reshaped(B, S, numVHeads, headVDim)", DERIVED, True,
    ...
    Row("gdn", "M<=2 conv concat([convState, qkv])", N_GDN,
        ...
        f"{Q35}:743", "concatenated([convState, qkv], axis: 1)", CITED, False,
        "concatenate_gpu = one copy_gpu_inplace per input (slicing.cpp:36-42)",
        f"{Q35}:1032", "concatenated([convState, qkv], axis: 1)"),
    Row("gdn", "M<=2 conv1d", N_GDN, lambda m, lim: 1 if m <= 2 else 0,
        f"{Q35}:746", "silu(conv1d(convInput))", CITED, False, "",
        f"{Q35}:1035", "silu(conv1d(convInput))"),
    ...
    Row("attn", "post-SDPA transpose+reshape copy", N_ATTN,
        lambda m, lim: 1 if m >= 2 else 0,
        f"{Q35}:1763", ".reshaped(B, L, -1)", DERIVED, True,
        "transposed [1,M,24,256] is row_contiguous only at M==1 (copy.cpp:216-232)"),
    Row("attn", "compiled sigmoid-multiply gate", N_ATTN, K(1),
        f"{Q35}:1765", "qwen35CompiledSigmoidMultiply(output, gate)"),
    Row("attn", "oProj", N_ATTN, qmv, f"{Q35}:1765", "oProj("),

    # ------------------------------------------------------------------- mlp
    Row("mlp", "fused gate_up quantizedMM (N = 34816)", N_LAYERS, qmv,
        f"{Q35}:1228", "quantizedMM("),
    Row("mlp", "compiled fused SwiGLU", N_LAYERS, K(1),
        f"{Q35}:1265", "qwen35CompiledFusedSwiGLU("),
    Row("mlp", "downProj", N_LAYERS, qmv, f"{Q35}:1268", "down"),

    # -------------------------------------------------------------- envelope
    Row("envelope", "QuantizedEmbedding lookup", 1, K(4),
        f"{Q35}:1935", "embedTokens(", DERIVED, False,
        "4-bit gather + scale/bias gathers + dequant"),
    Row("envelope", "per-block input/post-attention RMSNorm", N_LAYERS, K(2),
        f"{Q35}:1879", "inputLayerNorm(x)", CITED, False, "",
        f"{Q35}:1886", "postAttentionLayerNorm(h)"),
    Row("envelope", "per-block residual adds", N_LAYERS, K(2),
        f"{Q35}:1885", "let h = x + r", CITED, False, "",
        f"{Q35}:1886", "return h + (mlp as! UnaryLayer)"),
    Row("envelope", "final norm", 1, K(1), f"{Q35}:2346", "norm("),

    # ------------------------------------------------------------------ head
    Row("head", "LM head over ALL M rows -> [1, M, 248320]", 1, qmv,
        f"{Q35}:2349", "lmHead", CITED, False,
        "no row slicing: every draft row's full logit vector is produced"),
    Row("head", "top-two partial reduce", 1, K(1),
        f"{SESS}:1505", "linearTopTwoPartialKernel"),
    Row("head", "top-two finalize", 1, K(1),
        f"{SESS}:1512", "linearTopTwoFinalizeKernel"),
]
