...
# Rule 134 and Finding 255, for the ranked frame.
...
def main() -> None:
    ...
    for name, sites in SITE_READINGS.items():
        ...
        metrics[f"e135_finding252_m5_to_m4pro_ratio_{name}"] = \
            implied / per_fill_us
    for sites in (64, 128, 257):
        for src, us in (("m5fit", RIVAL_US_PER_ROUND / 64),
                        ("m4pro", per_fill_us)):
            pct = sites * us / RULE134_US_PER_ROUND_PER_PCT
            metrics[f"e135_idea3_ranked_pct_{sites}sites_{src}"] = pct
            metrics[f"e135_idea3_share_of_crown_gap_{sites}sites_{src}"] = \
                pct / GAP_TO_CROWN_TREE_PCT
            metrics[f"e135_idea3_share_of_bar_target_{sites}sites_{src}"] = \
    ...
    run = wandb.init(entity=entity, project=project, name=args.name,
                     job_type="local-cost-contrast", config=config)
    run.log(metrics)
