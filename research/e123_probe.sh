#!/usr/bin/env bash
# E123 -- one counterbalanced session for the extended instruction price ladder.
#
#   usage: research/e123_probe.sh TAG [extra probe args...]
#
# The measuring instrument is E118's `research/e118_qmv_probe.m`, unchanged.
# Only the arm set changes, which is the point: an extension measured by a
# different instrument would not be comparable with the ladder it extends.
#
...
tag="${1:?usage: e123_probe.sh TAG [args...]}"
shift
out_dir="research/out/${tag}"
arms_dir="/tmp/e123-arms"
bin="/tmp/e123_qmv_probe"
arms="$(python3 research/e123_arms.py --arm-list)"
[[ -n "${arms}" ]] || { echo "e123_probe: empty arm list" >&2; exit 1; }
mkdir -p "${out_dir}"
...
{
  echo "experiment=e123-price-ladder-completion-and-deletion-audit"
  echo "harness=local"
  echo "instrument=research/e118_qmv_probe.m (unchanged from E118)"
  echo "arms=${arms}"
  echo "args=$*"
