#!/usr/bin/env bash
# E217 Stage-0 numerics falsification gate.
#
#   research/e217_gate.sh TAG
#
# Compares the `coop` and `staged` grid mappings against the shipped `split`
# mapping at every fused decode cell and every `G == 2` width, on real packed
# 4-bit weights and real bfloat16 activations, as actual floating-point values.
# The expectation is bit-exact; one positive control per mapping proves the
# comparison can fail.
#
...
tag="${1:?usage: research/e217_gate.sh TAG}"
...

{
  ...
  echo "cells=${MLXFAST_E217_CELLS:-<all>}"
...
if [[ "${status}" -eq 0 ]]; then
  MLXFAST_RUN_E217_GATE=1 \
  MLXFAST_E217_GATE_OUT="${PWD}/${out}/gate.json" \
    swift test -c release --force-resolved-versions -Xswiftc -enable-testing \
      --skip-build --filter E217StagedMappingTests 2>&1 | tee "${out}/gate.log"
  status=${PIPESTATUS[0]}
fi
