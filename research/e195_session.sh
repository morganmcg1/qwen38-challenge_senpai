#!/usr/bin/env bash
# E195 cell-selective QMV width plan sections.
#
#   research/e195_session.sh <tag> [gate|probe|both]
#
...
tag="${1:?usage: e195_session.sh <tag> [gate|probe|both]}"
section="${2:-both}"
out="research/out/e195/${tag}"
mkdir -p "${out}"
...
publish_metallib() {
  ...
  if [[ -z "${source}" ]]; then
    echo "e195_session.sh: no mlx.metallib matches fingerprint ${want}." >&2
    echo "e195_session.sh: run tools/build-mlx-metallib.sh --all-build-roots" >&2
    return 1
  fi
  while IFS= read -r bundle_dir; do
    ...
    echo "e195_session.sh: published mlx.metallib into ${bundle_dir}"
...

{
  ...
  echo "experiment=e195-cell-selective-qmv-plan"
  echo "harness=local"
  ...
  echo "cells=${MLXFAST_E195_CELLS:-<all seven>}"
  echo "gate_widths=${MLXFAST_E195_GATE_WIDTHS:-<test-default>}"
  echo "probe_widths=${MLXFAST_E195_PROBE_WIDTHS:-<test-default>}"
  echo "blocks=${MLXFAST_E195_BLOCKS:-<test-default>}"
  echo "reps=${MLXFAST_E195_REPS:-<test-default>}"
...
if [[ "${section}" == "gate" || "${section}" == "both" ]]; then
  MLXFAST_RUN_E195_GATE=1 \
  MLXFAST_E195_GATE_OUT="${PWD}/${out}/gate.json" \
    swift test -c release --force-resolved-versions -Xswiftc -enable-testing \
      --filter E195CellSelectiveQMVTests 2>&1 | tee "${out}/gate.log"
  status=${PIPESTATUS[0]}
fi

if [[ "${status}" -eq 0 && ( "${section}" == "probe" || "${section}" == "both" ) ]]; then
  MLXFAST_RUN_E195_PROBE=1 \
  MLXFAST_E195_PROBE_OUT="${PWD}/${out}/probe.json" \
    swift test -c release --force-resolved-versions -Xswiftc -enable-testing \
      --filter E195CellSelectiveQMVTests 2>&1 | tee "${out}/probe.log"
  status=${PIPESTATUS[0]}
fi
