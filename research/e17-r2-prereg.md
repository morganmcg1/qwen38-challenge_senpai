...
Taking that step makes the round draft `depth + 1` tokens, which the target
verifies at width `M = depth + 2` (the `depth + 1` drafts plus the pending
primary token).

> **Convention: `h[i]` prices the step `depth i -> i+1` and buys verify width
> `M = i + 2`.**
...
### P1 -- the `S18` control goes deeper than r1's curve

r1's CURVE depth histogram on `english` (512 decode tokens, gate 3) was
`{1:2, 2:231, 3:13}` with `M>=5` share `0.00%`.

Predicted: the `S18` control on `af80b0fc` on `english` at 512 decode tokens is
**no shallower**, and shows a **non-zero `M>=5` share** (i.e. some depth>=3
rounds).

Honest attribution, stated up front: the simulator says the gate change 3 -> 2
contributes only ~0.015 mean depth (3.558 -> 3.573) because the width cap of 5
is rarely binding at these process parameters. Essentially all of the predicted
deepening comes from the **h vector** (flat 0.18 prices depth 2 and 3 at 0.18,
versus the curve's 0.2426 and 0.3754), worth ~1.2 mean depth
(2.388 -> 3.573). P1 is a test of the vector, not of the gate.

**Stop rule:** if the measured `S18` control is *shallower* than
`{1:2, 2:231, 3:13}`, or its `M>=5` share is `0.00%`, the depth model in
`e17_gate_sim.py` is wrong. r2 stops the sweep and reports that as the result.
