...
# TRACED, NON-TIMED dispatched-width census at the scored geometry.
#
#   research/e37-run.sh --golden ID [ID ...]   512-step serial goldens
#   research/e37-run.sh --census ID [ID ...]   traced 512-token --local-iterate
#
# COUNTS ONLY. Every run here sets MLX_QWEN_MTP_TRACE=1, which buys per-round
# file I/O inside the round, and MLXFAST_LOCAL_COOL_GATE=0. No number produced
# by this script is a timing measurement; meta.txt records both facts verbatim
# so no downstream reader can mistake one for a gate-qualified figure.
#
...
if [[ "${mode}" == "--golden" ]]; then
  files=()
  for id in "$@"; do files+=("$(prompt_file_for "${id}")"); done
