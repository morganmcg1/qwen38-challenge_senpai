...
# Session: PR #7 and PR #10 merged

Generator for everything numeric below: `research/knee_regime_reconciliation.py`
(no deps, <1 s). **If this file and that script disagree, the script wins.**

## 1. A merged win that banks nothing

PR #7 (qwen-askeladd) is merged and is worth **−1.90% ms/token** at 3-bit
(−1.42% at 2-bit), acceptance-neutral, emitted token streams byte-identical
across bit widths (sha256 `da92be8a…f65acf90`). **It changes the scored run by
exactly zero.**

- `Qwen35.swift` `draftHeadBits` reads `MLX_QWEN_MTP_DRAFT_BITS`, `else { return 4 }`.
- The only setter in the tree is `research/run-draft-bits-arm.sh:37`.
- The ranked workflow's `env:` block sets only `MLXFAST_*` names it defines itself.

The `MLX_` prefix was chosen deliberately (comment at `Qwen35.swift:2413-2414`)
because the harness strips `MLXFAST_*` from the sandboxed worker. Correct for an
experiment knob; it also guarantees the knob is unreachable in production.

> **A knob the scored run cannot reach is a measurement, not a win. Defaults are
> the only channel to the scoreboard.** Check the banking path *before* merging,
> not after. This is now a standing review question for every PR that adds a
> tunable.

PR #11 tests whether the default can be flipped: multi-prompt validation first,
then one arm on the unset-env default path.

## 2. Prediction 8 scored: direction confirmed, my statistic refuted

PR #10 (qwen-thorfinn) settled the crossrow verify kernel with two complementary
wrong-numerics arms. **ALU-bound at M≥4.** Arm 1 (ops −2.7×, bytes/loads/trips
held) cut time 24%/49% at M=4/8; arm 2 (unique bytes −4×, all ops held) moved it
0.6%/0.3%. Noise ±1.2% worst case. DCE closed by AIR load counts; **arm 2's
liveness closed by `row0_bitwise_matches_m1` 0/16**, which is the check that
distinguishes a real flat result from a silent no-op.

He then refuted the statistic in my own brief, and I reproduced every number:

| statistic | my brief | reproduced |
|---|---:|---:|
| `nominal × M` | 692 ± 5.6% | 692.04 ± 5.61% ✓ |
| "tighter" ratio vs flat `nominal` | 6.0× | 5.96× ✓ |
| honest ratio vs `H_C = nominal × ceil(M/4)` | *never computed* | **3.83×** ✓ |
| null calibration on synthetic pure-`H_C` data | *never computed* | **2.2×** ✓ |
| same, M=4..9 / M=1..9 | — | 2.3× / 1.2× ✓ |

Two distinct errors, worth separating:

1. **I scored my hypothesis against a straw competitor.** Nobody believed `H_A:
   flat nominal`. The campaign believed `H_C: the ceil(M/4) staircase` — the
   model I claimed to be killing. Reporting the margin over `H_A` as if it were
   over `H_C` inflated 3.8× into 6.0×.
2. **My test statistic had an uncalibrated null.** Run it on noise-free data
   *generated by `H_C` itself* and it still answers "ALU-bound, 2.2× tighter".
   A statistic that returns 2.2× on data from the hypothesis it rejects has a
   floor of 2.2×.

> **Score a hypothesis against the hypothesis you are actually trying to kill,
> and calibrate the statistic on data generated by that rival.** Neither step is
> expensive. Skipping both produced a number that was wrong by 57% in the
> direction that flattered me.

The M=1..9 flip to `H_C` is **not** evidence against the two-regime model —
`t(M) = max(t_bw, β·M)` requires the law to change at the knee, so `nominal × M`
must rise below it. But this cuts against me harder than it helps: the
invariance claim only ever covered the four points above the knee, and a 4-point
invariant with a 2.2× null floor is thin. **The conclusion rests on the arms, not
on my table.**

### Third advisor error: I handed over the wrong denominator

The brief said to report "against the 273 GB/s roofline". The cleanest
independent kill of the integer-stream model is that it implies **262.1 GB/s at
M=5 on a host whose measured achievable peak is 226.9 GB/s**. Against the 273
GB/s *spec* figure I supplied, 262.1 is merely implausible and the argument
evaporates. He got the decisive check by declining to use my denominator.

Independently corroborated across PRs: PR #7 measured STREAM peak
`227,128,791,836.97` B/s on the same host class. **227.1 vs 226.9 — 0.1% apart.**

> **Quote the achievable roofline, never the spec sheet.** A spec denominator
> hides exactly the impossibility results that are worth the most.

## 3. ★ PR #7 and PR #10 confirm each other across the knee

On its face these two merged results contradict: PR #7 says cutting bytes cuts
time 24–42% (bandwidth-bound); PR #10 says cutting bytes 4× moves time 0.3–0.6%
(ALU-bound). Both are right, because they sit on **opposite sides of one knee**.

Verified, not assumed: `QwenQMVCostCurveTests.sweepCompactDraftReadoutOverBits`
builds activations with `syntheticActivations(m: 1, k: 5120)` — **M = 1**.

| measurement | M | regime predicted by knee ∈ [2.99, 3.27] | observed |
|---|---:|---|---|
| PR #7 draft head readout | 1 | bandwidth-bound | flat GB/s (243.0/249.4/234.4), time ∝ bytes ✓ |
| PR #10 crossrow verify | 4, 8 | ALU-bound | bytes −4× → −0.6%/−0.3%; ops −2.7× → −24%/−49% ✓ |

Two students, two harnesses, two code paths, neither aware of the other's
result. **This is the strongest support the two-regime model has** — considerably
stronger than the table I built it from, which the same PR just discredited.

**Forward prediction, recorded and deliberately not funded:** extending the
low-bit trick to the *verify-side* head at M=4..9 buys ≈0, against 24–42% at the
draft head. A session was about to be spent on this. It no longer will be.

> **Two students' "contradictory" results are a reconciliation waiting to happen,
> and the reconciliation is usually worth more than either result.** Nobody but
> the advisor is positioned to notice, because neither student can see the other's
> branch.

## 4. Multi-prompt validation was always feasible — advisor debt, now cleared

PR #7 shipped a blocking caveat: a second prompt "which the local harness does
not offer". False, and the correction was mine to supply before he wrote it.

- **Override:** `benchmark-qwen-mtp.sh:103` (local-iterate/256) and `:107`
  (local-submit/1024) both read
  `public_golden_path="${MLXFAST_QWEN_MTP_LOCAL_GOLDEN_FIXTURE:-…}"`.
- **Generator:** `mlxfast-swift generate-golden --prompt-file PATH --weights PATH
  [--tokenizer PATH] --output PATH --name NAME --steps N`
  (`Sources/MLXFastCLI/main.swift:759`), whose comment states this is how
  `correctness_prompts/*` were produced. `--steps >= correctnessSteps` (64).
- **The join:** `benchmark-qwen-mtp.sh:547` seeds the MTP reference from
  `.cases[0].prompt_tokens`; `:541-544` validates it is a non-empty array;
  `:535-536` confirms the existing fixture's `prompt_tokens` *is* the
  tokenization of its `.txt` seed.
- If a `.benchmark` section is also demanded, `attach-benchmark-oracle` is pure
  file I/O, "needs no weights and no model" (`main.swift:698-709`).

Tracked local prompt inventory is exactly three files:
`public_longcopy_gate_english_512.txt` (2735 B), `_512_256.json`, `_512_1024.json`.

> **"Unverified" or "not offered by the harness" in a student's report is advisor
> debt.** A student who is wrong about what a tool can do stays wrong until
> someone with cross-branch vision checks. Two students have now shipped a
> blocking caveat that a ten-minute source read dissolved.

## 5. Measurement-integrity defect: `benchmark-qwen-mtp.sh` never builds

It drives the CLI directly and enters `benchmark.sh` only via
`--local-cool-gate-only`, which returns before the build gate
(`benchmark.sh:1830`). It contains no `swift build`. Its own comment at `:144`
concedes the branch "returns before the metallib check, the builds".

**Anything run through it times whatever binary already sat in `.build/release`.**

Exposure audit:

| caller | verdict |
|---|---|
| `research/run-gate-arm.sh` (alphonse) | **clean** — `build_products()` builds both binaries, asserts arm constants pre-build, post-reapply, post-build |
| `research/run-draft-bits-arm.sh` (askeladd) | **clean** — builds the worker, refuses to time a binary whose `strings` lack `MLX_QWEN_MTP_DRAFT_BITS` |
| `research/run-amdahl-measurement.sh:40` | **EXPOSED** — calls it with no build step at all |

`run-amdahl-measurement.sh` produced **PR #3's seed-prefill and depth anchors,
including the refuted `C(8) = 161.0 ms`.** Stale-binary is therefore a **named
third hypothesis** for that discrepancy, alongside mis-labelled depth. Note that
`run-draft-bits-arm.sh` *wraps* the defective script and is safe only because it
pre-builds — the wrapper is doing real work.

**Trap for PR #11:** a `strings`-based tripwire goes blind the moment a *default*
is flipped, because the stale and fresh binaries both contain the string. The
robust replacement is a `sha256` of the built worker recorded per arm.

## 6. Flag C reconciled with "measured rather than pinned"

`fixtures/qwen3_8_27b_mtp_track.json` `/scoring_semantics/per_prompt_score` says
the serial depth-0 leg is "measured rather than pinned". This does **not**
contradict Flag C. Two different questions:

- **Which tree runs the serial leg?** The separately pinned prebuilt baseline
  workspace — `.github/workflows/qwen-mtp-ranked-benchmark.yml:~2957-2966` passes
  `--candidate "${MLXFAST_JOB_WS}"` and `--baseline
  "${MLXFAST_QWEN_MTP_BASELINE_RESOLVED}"` as **distinct trees**. (Flag C.)
- **Is its timing read live?** Yes — in the same thermally-gated session, order
  alternating, not read back from `baseline-calibration.json`.

Both true. Consequence unchanged: **general target/kernel/prefill wins are fully
scored, and absolute candidate wall time is the true signal.** Local same-build
ratio remains a decoy *except* for schedule-or-head changes.

Corroborating detail found this session, `Constants.swift:375-376`: "The depth-0
leg does prompt-INDEPENDENT work (512 plain forwards), which is why one pooled
denominator serves all 8 timed prompts."

## 7. Scoring facts read from the track fixture

- `pairs_per_prompt = 1` ⇒ ranked k=1, 8 prompts × 1 pair = 8 pairs / 16 timed phases.
- Per-prompt `noop_decode_speedup_spread_pct` 0.14–0.44%.
- The 8 `timed_prompt_pool` entries are **reference-rows goldens fetched from
  R2**, not prose, each seeded from a different 512-token prose seed via
  `mtp-verify --emitted <plan> --generate 513`.
- Serial denominator band (`fixtures:261` mean `0.037994794617407023`, band
  0.95/1.05) is enforced by box-owned `/opt/bench-runner/measure-qwen-mtp-job.sh`.
  **Not a hazard we can create** — a band death is a box event; re-run it.

## 8. Acceptance break-even for the low-bit draft head

Control `a₀ = 0.8902691511387164`; tokens/round `= 1 + 430/82 = 6.2439`;
readouts/round `= 483/82 = 5.8902`; relative sensitivity `5.890/6.2439 = 0.943`.

| arm | time saved | break-even drop | **acceptance floor** |
|---|---:|---:|---:|
| 3-bit | 0.7574% | 0.00803 | **0.88224** |
| 2-bit | 1.3269% | 0.01407 | **0.87620** |

**These floors are soft and must never be quoted without the caveat:** the model
prices a rejected draft as one lost token, but a rejection also forces rollback
and re-forward, so true degradation is worse than linear in `a`. An arm landing
*near* its floor is a loss.
...
## 10. Open, funded and unfunded

- **`qmm_splitk` contradiction (new, unfunded, best-motivated kernel question).**
  ALU-bound predicts that padding verify width 9→10 — which crosses
