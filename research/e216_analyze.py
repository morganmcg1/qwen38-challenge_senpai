#!/usr/bin/env python3
"""Reduce an E216 Stage-1 palindrome session to its decision statistics.

    usage: research/e216_analyze.py SESSION_DIR [OUT_JSON]

Arms: `off` (shipped `split` grid) and `coop` (the paired grid that places both
token-column groups of one output-row slice in a single threadgroup). Both
mappings keep threadgroup count, threadgroup shape and per-simdgroup geometry
identical, so the contrast is weight-stream locality alone.

Moved widths are the staged G == 2 entries m = 6, 7, 8 and 9. The m <= 5 band is
G == 1 in both arms and is the flat control: a non-zero effect there is a
mechanism side effect (coop halves the per-threadgroup output-row footprint, so
the L2 working set changes even where the weight-pass count does not) and is
reported separately rather than absorbed into the census weighting.
...
def load_leg(directory: pathlib.Path) -> dict:
    ...
    return {
        "dir": str(directory),
        "arm": meta["e216_arm"],
        "leg": int(meta["e216_leg"]),
        "expected_plan": meta["e216_expected_plan"],
        "observed_plans": meta["e216_observed_plans"],
        "qmv_coop_launches": int(meta["e216_qmv_coop_launches"]),
        "qmv_split_launches": int(meta["e216_qmv_split_launches"]),
        "qmv_split_g2_launches": int(meta["e216_qmv_split_g2_launches"]),
...
def main() -> int:
    ...
    for leg in legs:
        ...
        # RULE: a coop leg proves the scored path by dispatching every G == 2
        # cell through the paired grid, so no split G == 2 launch may remain.
        if leg["arm"] == "coop":
            if leg["qmv_coop_launches"] <= 0:
                problems.append(
                    f"leg {leg['leg']}: coop arm made no coop launch"
                )
            if leg["qmv_split_g2_launches"] != 0:
                problems.append(
                    f"leg {leg['leg']}: coop arm left "
                    f"{leg['qmv_split_g2_launches']} split launches at G == 2"
                )
        else:
            if leg["qmv_coop_launches"] != 0:
                problems.append(
                    f"leg {leg['leg']}: off arm made "
                    f"{leg['qmv_coop_launches']} coop launches"
                )
            if leg["qmv_split_g2_launches"] <= 0:
                problems.append(
                    f"leg {leg['leg']}: off arm reached no G == 2 cell, so the "
                    "session cannot measure the mechanism"
                )
        ...
        "legs": [
            {
                key: leg[key]
                for key in (
                    "leg", "arm", "observed_plans", "qmv_coop_launches",
                    "qmv_split_launches", "qmv_split_g2_launches",
    ...
    for arm, arm_legs in by_arm.items():
        ...
        if identical and not problems:
            ...
            # The promotion rule's flat band. Coop halves the per-threadgroup
            # output-row footprint at every width, so a move here is a mechanism
            # side effect on the shared L2 working set, not weight-pass value.
