...
def main() -> int:
    ...
    # 6 REPLICATION ---------------------------------------------------------
    #
    # F15 replicates C's coefficient on a second receipt pair to within 2.4 %
    # and reads that as support for the law. A replication only discriminates
    # if a rival law would have produced a different number. The rival here is
    # "one flat saving per DRAFTING round", and the drafting profile is a
    # property of the prompt pool, so it is the same in every pair. Fitting C
    # to noiseless data generated by the rival shows what C reports when the
    # rival is true.
    print("\n################ 6 does the replication discriminate? ################")
    flat = fit([r["saving"] / r["fraction"] for r in drafting], [])["beta"][0]
    synthetic = [flat * r["fraction"] for r in table]
    synth_c = fit(synthetic, [[math.log(r["mbar"]) for r in table]],
                  intercept=False)
    print("rival law: saving = %.1f us * drafting fraction, exact, no noise"
          % flat)
    print("fitting C to that rival gives a = %.1f, uncentered R2 %.4f"
          % (synth_c["beta"][0], synth_c["r2_uncentered"]))
    print("C fitted to the real receipts gives a = %.1f" % coefficient)
    print("the two differ by %.1f %%; the drafting profile is fixed across "
          "submission pairs, so any pair reproduces this coefficient whether "
          "the law is logarithmic or flat-per-drafting-round"
          % (100.0 * abs(synth_c["beta"][0] - coefficient) / coefficient))
