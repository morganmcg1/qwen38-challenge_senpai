...
#
#   research/run-draft-bits-phase3.sh TAG_PREFIX [TOKENS] [BASE_SHA] [ORDER]
#
...
#
# Each arm is also preceded by a bounded settle to a common GPU temperature.
# This is NOT benchmark.sh's 40C cool gate and cannot be: the host idles at a
# ~40.7C inlet-air-bound floor, so that gate always aborts here. Every second
# reported by this driver is temperature-EQUALIZED, not gate-qualified, and must
# be labelled a directional screen.
set -euo pipefail

prefix="${1:?usage: run-draft-bits-phase3.sh TAG_PREFIX [TOKENS] [BASE_SHA] [ORDER]}"
tokens="${2:-256}"
base_sha="${3:-}"
order="${4:-4,3,3,4}"
...
settle_target_c="${MLXFAST_SETTLE_TARGET_C:-44.0}"
settle_max_s="${MLXFAST_SETTLE_MAX_S:-120}"
...
settle() {
  ...
  while :; do
    ...
    if awk -v a="${t}" -v b="${settle_target_c}" 'BEGIN{exit !(a<=b)}'; then
      echo "phase3-settle: pos=${pos} reached=${t}C target=${settle_target_c}C waited=${waited}s"
      return 0
    fi
    if [[ "${waited}" -ge "${settle_max_s}" ]]; then
      echo "phase3-settle: pos=${pos} STALLED at=${t}C target=${settle_target_c}C waited=${waited}s"
      return 0
    fi
    sleep 10
    waited=$((waited + 10))
...
echo "run-draft-bits-phase3: prefix=${prefix} tokens=${tokens} order=${order}"
