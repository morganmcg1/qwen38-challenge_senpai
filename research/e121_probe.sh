#!/usr/bin/env bash
# E121 rung 2 -- share the activation chunk-sum term across the two simdgroups
# of the wide QMV threadgroup, with the shared path gated to NA <= 4.
#
#   usage: research/e121_probe.sh TAG [extra probe args...]
#
...
tag="${1:?usage: e121_probe.sh TAG [args...]}"
shift
out_dir="research/out/${tag}"
arms_dir="/tmp/e121-arms"
bin="/tmp/e121_rate_probe"
...
arms="${E121_ARMS:-a_base,a_scaffold,x_split_pred,x_min_ask,g_min_ask,g_split_pred_pp,g_split_pred}"
mkdir -p "${out_dir}"
...
rm -rf "${arms_dir}"
python3 research/e121_arms.py --emit "${arms_dir}" 2>&1 \
  | tee "${out_dir}/arms.log" || exit 1

# E104's harness, unchanged and shared with E110 and E118, so the arm set, the
# entry-point name and the width list are arguments and the instrument is the
# one that produced the numbers this rung has to compare against.
...
"${bin}" --dir "${arms_dir}" --out "${PWD}/${out_dir}/rate.json" \
  --fn 'e121_iso_na%d' --arms "${arms}" --widths 2,3,4,5 \
...
{
  echo "experiment=e121-cross-simdgroup-activation-sum-sharing-shipped"
  echo "harness=local"
  echo "arms=${arms}"
  echo "args=$*"
