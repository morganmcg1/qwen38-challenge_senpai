...
# Every multi-chunk instantiation above spills, and every one of them also fails
# device parity, so the two facts are confounded. This arm carries the same
# chunk rewrite on the smallest row block, which allocates far below the ceiling
# and is expected not to spill while still emitting more than one chunk. It
# separates a defective rewrite from a miscompile on the spilled path.
ARMS.append(("rps1mc4", 1, True, mchunk_rewrites(4)))
