...
alignment does hold and the code can assert it.
  ...
  fma       the alternative to the assigned hypothesis: the cell is bound by
            ARITHMETIC issue slots, not by operand loads. `-fno-fast-math`
            stops the compiler contracting anything, and the AIR census
            confirms `fma = 0` in every arm above, so the k-loop spends
            4 fmul + 4 fadd per (row, i) where 4 fma + 1 fadd would do. Each
            product is exact in binary32, so the contraction is bit-identical.
  fmawxvec  fma + wvec + xvec: the arithmetic cut and the load cut together.
...
FMA = ("contract the four exact nibble products into fused multiply-adds",
       QDOT_SEPARATE, QDOT_FMA)
TGX = [
...
def arms() -> dict[str, list[tuple[str, str, str]]]:
    ...
    return {
        ...
        "e69fma": common("e69fma") + [FMA],
        "e69fmawxvec": common("e69fmawxvec") + [FMA, WVEC, XVEC],
    }
